<template>
  <div>
    <headerTab msg=""></headerTab>
    <div class="container">
      <div class="home-main box-sizing">
        <div class="h-main-left inline-block">
          <div class="home-model-header">
            <div class="inline-block home-head-title">申请咨询师</div>
          </div>
          <div class="msg-detail box-sizing">
            <div class="inline-block head-msg">
              <img :src="user.headImage!=null?(user.headImage.indexOf('base64')==-1?head_src+user.headImage:user.headImage):''" onerror="javascript:this.src='./static/img/expert-head.png';" alt="">
              <div>
                上传真实的头像
            ...
            <ul class="inline-block msg-ul">
              <li class="msg-li box-sizing">
                ...
                <div class="inline-block">
                  <input type="text" class="short-input box-sizing" maxlength="5" v-model="user.realName" placeholder="昵称不超过5个字符">
                </div>
                <span><span class="orange">* </span>真实姓名：</span>
                <div class="inline-block">
                  <input type="text" class="short-input box-sizing" maxlength="5" v-model="user.userName" placeholder="真实姓名不超过5个字符">
                </div>
              </li>
              <li class="msg-li box-sizing">
                <span>性别：</span>
              ...
              <li class="msg-li">
                ...
                <div class="inline-block">
                  <v-distpicker :province="user.province?(user.province=='北京'||user.province=='上海'||user.province=='天津'||user.province=='重庆'?user.province+'市':user.province+'省'):''" :city="user.province?(user.province=='北京'||user.province=='上海'||user.province=='天津'||user.province=='重庆'?user.province+'城区':user.address+'市'):''" :area="user.province=='北京'||user.province=='上海'||user.province=='天津'||user.province=='重庆'?user.address:''"  @province="onChangeProvince" @city="onChangeCity" @area="onChangeArea"></v-distpicker>
                </div>
              </li>
              <li class="msg-li">
                <span>职务：</span>
              ...
              <li class="msg-li">
                ...
                <div class="inline-block jdal-img-show" v-for="item in user.cases">
                  <img :src="item!=null?(item.indexOf('base64')==-1?cert_src+item:item):('')" alt="">
                ...
                <div class="inline-block">
                  <div class="ly-div box-sizing">
                    ...
                    <div class="inline-block box-sizing zt">
                      <span :title="adep.indexOf(item.uuid)!=-1?'点击取消':'点击选中'" class="inline-block ly-list cursor" :class="adep.indexOf(item.uuid)!=-1?'blue':''" v-for="item in zt" @click="checkbox_click($event,'.zt',item)">
                  ...
                  <div class="ly-div box-sizing">
                    ...
                    <div class="inline-block box-sizing sz">
                       <span :title="adep.indexOf(item.uuid)!=-1?'点击取消':'点击选中'" class="inline-block ly-list cursor" :class="adep.indexOf(item.uuid)!=-1?'blue':''" v-for="item in sz" @click="checkbox_click($event,'.sz',item)">
                        <!--<input type="checkbox" :checked="adep.indexOf(item.uuid)!=-1" @change="$event.target.checked?adep.push(item.uuid):remove(adep,item.uuid)" class="zt">-->
                       {{item.name}}
                      </span>
                  ...
                  <div class="ly-div box-sizing">
                    ...
                    <div class="inline-block box-sizing hy">
                       <span :title="adep.indexOf(item.uuid)!=-1?'点击取消':'点击选中'" class="inline-block ly-list cursor" :class="adep.indexOf(item.uuid)!=-1?'blue':''" v-for="item in hy" @click="checkbox_click($event,'.hy',item)">
                        <!--<input type="checkbox" :checked="adep.indexOf(item.uuid)!=-1" @change="$event.target.checked?adep.push(item.uuid):remove(adep,item.uuid)" class="zt">-->
                       {{item.name}}
                      </span>
              ...
              <li class="msg-li">
                ...
                <div class="sfz-show inline-block box-sizing" v-show="user.idCardFront!=''&&user.idCardFront!=null">
                  <img :src="user.idCardFront!=null?(user.idCardFront.indexOf('base64')==-1?cert_src+user.idCardFront:user.idCardFront):''" alt="">
                  <span class="inline-block cursor" @click="user.idCardFront=''">x</span>
                </div>
                <div class="inline-block sfz-pic box-sizing" v-show="user.idCardFront==''||user.idCardFront==null">
                  <img src="../../../static/img/sfz-front.png" alt="">
                  <div>点击上传身份证正面</div>
                  <input type="file" class="sfz1" @change="img_upload('.sfz1','idCardFront')">
                </div>
                <div class="sfz-show inline-block box-sizing" v-show="user.idCardBack!=''&&user.idCardBack!=null">
                  <img :src="user.idCardBack!=null?(user.idCardBack.indexOf('base64')==-1?cert_src+user.idCardBack:user.idCardBack):''" alt="">
                  <span class="inline-block cursor" @click="user.idCardBack=''">x</span>
                </div>
                <div class="inline-block sfz-pic box-sizing" v-show="user.idCardBack==''||user.idCardBack==null">
                  ...
                  <div class="zs-list">
                    ...
                    <div>
                      <div class="kjs-up kjs-show inline-block box-sizing" v-for="item in user.accountants">
                        <img :src="item.indexOf('base64')==-1?cert_src+item:item" alt="">
          ...
          <div class="sub-btn-group">
            ...
            <div class="inline-block btn-back cursor" @click="$router.push({name:'mineWorkbench'})">返回</div>
          </div>
        </div>
        <div class="h-main-right inline-block box-sizing">
          <div class="msg-notice">
            <div class="inline-block">温馨提示：</div>
    ...
    export default {
      ...
      mounted(){
        ...
        this.ajax_nodata(this.http_url.url+"/user/message",function(data){
          that.user=data;
          // if(data.province==null||data.province=='null'){
          //   that.user.province='北京'
          // }
          // if(data.address==null||data.address=='null'){
          //   that.user.address='北京'
          // }
