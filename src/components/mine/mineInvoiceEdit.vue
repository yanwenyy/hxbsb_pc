      ...
      <div class="mine-right inline-block box-sizing">
        <div class="mine-data-head box-sizing">
          <div class="home-model-header">
            <div class="inline-block home-head-title"><span class="inline-block span-blue-line"></span>消费信息</div>
          </div>
          <div class="mine-data-user-msg">
            <div class="inline-block">
              消费内容：{{$route.query.msg.productType}}
            </div>
            <div class="inline-block">
              消费时间：{{format($route.query.msg.paymentDate)}}
            </div>
            <div class="inline-block">
              消费金额：{{$route.query.msg.currentPrice}}元
            </div>
          </div>
        </div>
        <div class="mine-right-body">
          <div class="mine-data-body box-sizing">
            <div class="home-model-header" style="margin-top: 0;">
              <div class="inline-block home-head-title"><span class="inline-block span-blue-line"></span>发票填写</div>
            </div>
            <ul>
              <li>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      sub:function(){
        var that=this;
        if(this.companyName==''||this.taxNum==''||this.receiverPhoneNum==''||this.email){
          ...
          this.ajax(this.http_url.url+"/pay/addInvoice",{
            ...
            "uuid":this.$route.query.msg.uuid
          },function(data){
            if(data.code==1){
