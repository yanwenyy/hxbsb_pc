      ...
      <div class="container">
        ...
        <div class="mine-right inline-block box-sizing">
          <div class="home-model-header">
            <div class="inline-block home-head-title"><span class="inline-block span-blue-line"></span>抢答详情</div>
          </div>
          <div class="box-sizing mine-ques-detail">
            ...
            <div class="home-model-header home-model-header2" v-show="answewrUsers.length>1">
            ...
            <div class="answer-group" v-for="item in answewrUsers">
              <div class="home-model-header home-model-header2" v-show="item.status!=6&&answewrUsers.length==1">
                <div class="inline-block home-head-title"><span class="inline-block span-blue-line"></span>回答</div>
                <div class="error-eidt-btn inline-block" v-if="jc_btn_status"  @click="jc_status=!jc_status"><img src="../../../static/img/error-eidt-btn.png" alt="">我要纠错</div>
              </div>
              <div class="home-model-header home-model-header2" v-show="item.status==6">
          ...
          <div class="back"><div class="cursor" @click="$router.push({name:'mineAnswer'})">返回</div></div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      //回答者和提问者信息
      this.ajax(this.http_url.url+"/question/otherQuestion",{
        "questionUuid":this.$route.query.id,
      },function(data){
        that.questionUser=data.question;
        that.answewrUsers=data.answerUsers;
        that.changerAnswer=data.changerAnswer;
        if(data.answerUsers[0].status==2){
          that.jc_btn_status=true;
          that.comment_id=data.answerUsers[0].uuid;
        }
      });
      //行业,税种,专题
